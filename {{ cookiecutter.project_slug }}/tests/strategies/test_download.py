...
def test_download_file(filename: str, mediaType: str, static_files: Path) -> None:
    ...
    download_initialize = FileStrategy(config).initialize()
    download_get = FileStrategy(config).get()

    content: bytes = DataCache().get(download_get.key)

    # `download_initialize` is not actually a dictionary, but rather an AttrDict
