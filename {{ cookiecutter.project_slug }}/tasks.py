...
def create_api_reference_docs(context, pre_clean=False, pre_commit=False):
    ...
    package_dir = TOP_DIR / "{{ cookiecutter.package_name }}"
    ...
    for line in dependencies:
        ...
        if match is None:
            ...
            if fail_fast:
                sys.exit(msg)
            print(msg)
        full_dependency_name: str = match.group("full_dependency")
        package: str = match.group("package")
        operator: str = match.group("operator")
        version: str = match.group("version")
        version_req_op: "Optional[str]" = match.groupdict().get("version_req_op")
        version_req: "Optional[str]" = match.groupdict().get("version_req")
        ...
        # Check version requirement (if any)
        if version_req:
            if not version_req_op:
                msg = f"No operator for version requirement: {version_req!r}"
                if fail_fast:
                    sys.exit(msg)
                print(msg)
            else:
                for index, version_part in enumerate(version_req.split(".")):
                    if version_req_op == ">":
